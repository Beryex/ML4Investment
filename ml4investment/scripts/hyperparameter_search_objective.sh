...
for objective in $OBJECTIVE; do
    export WANDB_RUN_GROUP="objective-${objective}-$(date +%Y%m%d-%H%M%S)"
    echo "WANDB_RUN_GROUP set to: ${WANDB_RUN_GROUP}"

    export TRAIN_OBJECTIVE=$objective
    echo "TRAIN_OBJECTIVE set to: ${TRAIN_OBJECTIVE}"
    ...
    for start_year in $START_YEARS; do
        ...
        for start_month in $START_MONTHS; do
            ...
            RESULT_DIR="objective-${objective}/${start_year}-${start_month}"
            CONFIG_DIR="config"
            DATA_DIR="data"
            LOG_DIR="logs"
