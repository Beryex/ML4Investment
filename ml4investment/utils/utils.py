...
def update_gains(
    ...
    daily_gain_optimal = sum((1 + y_dict[backtest_day_index][s]) * weight_optimal for s in actual_optimal_stocks)
    ...
    daily_gain_actual  = sum((1 + y_dict[backtest_day_index][s]) * w for (s, w) in predict_top_stock_and_weights_list)
...
def get_detailed_static_result(model: lgb.Booster,
                               X_dict: dict,
                               y_dict: dict,
    ...
    for i in range(day_number):
        sorted_stock_gain_prediction = sorted(y_predict_dict[i].items(), key=lambda x: x[1], reverse=True)
        sorted_stock_gain_actual = sorted(y_dict[i].items(), key=lambda x: x[1], reverse=True)
        
        gain_predict, gain_actual, gain_optimal, daily_gain_predict, daily_gain_actual, daily_gain_optimal, cur_optimal_stocks, cur_actual_optimal_stocks = update_gains(
    ...
    if verbose:
        ...
        detailed_table.field_names = [
            "Day", "Predict daily gain", "Actual daily gain", "Optimal daily gain",
            "Predict optimal stock to buy", "Actual optimal stock to buy", "Overall Actual Gain"
        ]
        for res in daily_results_for_table:
            pred_stocks_str = [f"{s} ({settings.STOCK_SECTOR_ID_MAP.get(s, 'N/A')})" for s in res['predict_optimal_stocks']]
            actual_stocks_str = [f"{s} ({settings.STOCK_SECTOR_ID_MAP.get(s, 'N/A')})" for s in res['actual_optimal_stocks']]
            
            row = [
                res["day"],
                f"{res['daily_gain_predict']:+.2%}", f"{res['daily_gain_actual']:+.2%}", f"{res['daily_gain_optimal']:+.2%}",
                pred_stocks_str, actual_stocks_str, f"{res['cumulative_gain_actual']:+.2%}"
            ]
            detailed_table.add_row(row, divider=True)
            
        detailed_table.add_row(["Overall", f"{gain_predict:+.2%}", f"{gain_actual:+.2%}", f"{gain_optimal:+.2%}", "N/A", "N/A", "N/A"], divider=True)
        ...
        backtest_table.field_names = [
            "Backtest Trading Day Number", "Predict overall gain", "Actual overall gain", "Optimal overall gain", "Efficiency"
        ]
        efficiency = (gain_actual / gain_optimal) if gain_optimal != 0 else 0
        backtest_table.add_row([day_number, f"{gain_predict:+.2%}", f"{gain_actual:+.2%}", f"{gain_optimal:+.2%}", f"{efficiency:.2%}"], divider=True)
    ...
    detailed_static_table.field_names = [
        "MAE", "MSE", "Sign Accuracy", "Precision", "Recall", "F1 Score", "Actual Gain"
    ]
    row = [
        f"{mae_overall:.7f}", f"{mse_overall:.7f}", f"{sign_acc_overall*100:.2f}%",
        f"{precision_overall*100:.2f}%", f"{recall_overall*100:.2f}%", f"{f1_overall*100:.2f}%",
        f"{gain_actual:+.2%}"
    ]
